...
import { ChevronLeft, Search, Calendar, TrendingUp } from 'lucide-react';
...
export default function DueIn30DaysPage() {
  ...
  const totalAmount = useMemo(
    () => dueUpcomingInvoices.reduce((sum, inv) => sum + (inv.remainingBalance || 0), 0),
    [dueUpcomingInvoices]
  );
    ...
    <SidebarInset className="flex-1 overflow-auto bg-background">
      <header className="sticky top-0 z-10 flex h-20 shrink-0 items-center gap-4 border-b bg-background/80 backdrop-blur-md px-4 md:px-8">
        ...
        <div className="flex flex-col">
          <h2 className="text-lg font-bold font-headline text-slate-900 tracking-tight">Due in 30 Days</h2>
          <p className="text-xs text-slate-500 mt-0.5">{dueUpcomingInvoices.length} invoices upcoming</p>
        </div>
      </header>

      <main className="p-4 md:p-8 space-y-6">
        {/* Summary Card */}
        <Card className="border-none shadow-sm ring-1 ring-slate-100 rounded-2xl bg-gradient-to-br from-amber-50 to-amber-50">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-2">Total Amount Due</div>
                <div className="text-3xl font-black text-amber-700">{formatCurrency(totalAmount)}</div>
              </div>
        ...
        <Card className="border-none shadow-sm ring-1 ring-slate-100 rounded-2xl overflow-hidden">
          <CardHeader className="pb-4 border-b">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Upcoming Bills ({dueUpcomingInvoices.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {dueUpcomingInvoices.length === 0 ? (
              <div className="p-12 text-center">
                <p className="text-slate-500 text-sm">No bills due in the next 30 days.</p>
              </div>
            ) : (
